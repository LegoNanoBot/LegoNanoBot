...
def test_agent_enables_supervisor_routing_when_supervisor_enabled(mock_agent_runtime):
    mock_agent_runtime["config"].supervisor.enabled = True

    result = runner.invoke(app, ["agent", "-m", "hello"])

    assert result.exit_code == 0
    assert mock_agent_runtime["agent_loop_cls"].call_args.kwargs["routing_strategy"] is not None
